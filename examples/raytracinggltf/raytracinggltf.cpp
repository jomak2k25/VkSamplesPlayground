...
void VulkanExample::createRayTracingPipeline()
{
	...
	std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
		...
		vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 2),
		// Binding 3: Ray tracing normal Output
		vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_MISS_BIT_KHR, 3),
		...
		vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_ANY_HIT_BIT_KHR, 5),
...
void VulkanExample::createDescriptorSets()
{
	...
	for (auto i = 0; i < maxConcurrentFrames; i++) {
		...
		VkDescriptorImageInfo additionalStorageImageDescriptors[2] =
		{
			{ VK_NULL_HANDLE, additionalStorageImages[0].view, VK_IMAGE_LAYOUT_GENERAL },
			{ VK_NULL_HANDLE, additionalStorageImages[1].view, VK_IMAGE_LAYOUT_GENERAL }
		};
...
void VulkanExample::handleResize()
{
	...
	for (auto i = 0; i < maxConcurrentFrames; i++) 
	{
		VkWriteDescriptorSet resultImageWrites[3];
		resultImageWrites[0] = vks::initializers::writeDescriptorSet(descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &storageImageDescriptor);
		resultImageWrites[1] = vks::initializers::writeDescriptorSet(descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &additionalStorageImageDescriptors[0]);
		resultImageWrites[2] = vks::initializers::writeDescriptorSet(descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &additionalStorageImageDescriptors[1]);
		vkUpdateDescriptorSets(device, 1, resultImageWrites, 0, VK_NULL_HANDLE);
	}
	resized = false;
}
